...
const UserQuery = () => {
  ...
  const fetchData = () => {
    axios
      .get("http://localhost:1783/api/getcontactus", { headers: { "Authorization": `Bearer ${userToken}` } })
      .then((res) => {
        setQueries(res.data);
      })
      .catch((err) => {
  ...
  const handleDeleteCategory = (queryId) => {
    axios
      .delete(` http://localhost:1783/api/deletecontactus/${queryId}`, { headers: { "Authorization": `Bearer ${userToken}` } })
      .then((res) => {
        fetchData();
