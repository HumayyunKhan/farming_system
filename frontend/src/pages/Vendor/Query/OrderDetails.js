...
const Orders = () => {
  ...
  const fetchData = () => {
    axios
      .get(" http://localhost:1783/api/getorder", { headers: { "Authorization": `Bearer ${userToken}` } })
      .then((res) => {
        setOrders(res.data);
      })
  ...
  const handleDeleteProduct = (orderid) => {
    axios
      .delete(` http://localhost:1783/api/deleteorder/${orderid}`, { headers: { "Authorization": `Bearer ${userToken}` } })
      .then((res) => {
        fetchData();
